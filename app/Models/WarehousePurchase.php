...
class WarehousePurchase extends Model  implements Auditable
{
    ...
    {
        return $purchases = WarehousePurchase::select('warehouser_purchase.*',DB::raw('DATE(warehouser_purchase.p_date) as date_only'))->orderBy('id', 'desc')->where('ship_status',1)->paginate(15);
    }

    public function getPurchaseDataPending()
    {
        return $purchases = WarehousePurchase::select('warehouser_purchase.*',DB::raw('DATE(warehouser_purchase.p_date) as date_only'))->orderBy('id', 'desc')->where('ship_status',0)->paginate(15);
