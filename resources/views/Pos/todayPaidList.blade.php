...
@endphp
@section('title', 'Today Paid');
@section('body')
    <p class=" text-2xl">Today Paid Credit amount</p>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-5">
            ...
            <tbody>
                @forelse ($CashSaleData as $item)
                    <tr
                        ...
                        <td class="px-6 text-center  py-4 depost_paid">
                            {{ $item->deposit_paid }}
                        </td>
                        <td class="px-6 text-center  py-4">
                            {{ $item->credit_paid }}
       ...
       <div class=" flex justify-end">
        <span>Total: </span>
        <span id="credittotal"></span>
       </div>
    </div>
    <script>
          var depositamt = document.getElementsByClassName("depost_paid");
         
        var totoal = 0;
          for (let index = 0; index < depositamt.length; index++) {
            totoal +=  parseInt(depositamt[index].innerText )
          
          }
          document.getElementById('credittotal').textContent = totoal;
    </script>
