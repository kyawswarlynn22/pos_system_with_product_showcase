...
@section('body')
    <p class=" text-2xl">Edit CashSale</p>
    <form action="/preordersale/{{ $PreorderSale->id }}" method="post">
        @csrf
        @method('put')
        <div class="mt-3 rounded-lg shadow-lg p-5">
            <div class="row">
                <div class="col-xs-6 col-sm-6 col-md-6 ">
                    <span> {{ $PreorderSale->pur_date }} </span>
                </div>
                <div class="col-xs-6 col-sm-6 col-md-6 text-right">
                    <span>Invoice No: {{ $PreorderSale->id }}</span>
                </div>
            </div>
                ...
                <div class="mb-6 w-full">
                    ...
                    <select name="customer" id="customer"
                        ...
                        @forelse ($customerList as $item)
                            <option value="{{ $item->id }}" @if ($item->id == $PreorderSale->customers_id) selected @endif>
            ...
            <div class="card-body">
                <table id="products_table" class="w-full text-sm text-left text-gray-500 rounded-lg dark:text-gray-400">
                    ...
                    <tbody id="new">
                        @forelse ($PreorderSaleDetails as $item)
                            <tr
                ...
                <div class="mt-5 flex items-center justify-end  ">
                    ...
                    <input type="number" class="rounded-lg font-semibold text-lg w-28 border-transparent" name="discount"
                        value="{{ $PreorderSale->discount }}" id="discount">
                </div>
                <div class="mt-5 flex items-center justify-end  ">
                    ...
                    <input type="number" readonly class="rounded-lg font-semibold text-lg w-28 border-transparent"
                        name="grandtotal" value="{{ $PreorderSale->grand_total }}" id="gtotal">
                </div>
                <div class=" mt-28 mb-5 flex flex-col">
                    <label for="description">Remarks</label>
                    <textarea name="remark" id="" cols="100" rows="2">{{ $PreorderSale->remark }}</textarea>
                </div>
